...
class FragmentInjector:
    ...
    def traverse(self, node):
        ...
        LOG.debug("  " * len(self.open_tag_stack) + "Traversing node of type: %s", type(node))
...
class FragmentInjectorV2:
    ...
    def inject(self):
        ...
        if self.new_children:
            for child in self.new_children: self.tag.append(child)
    ...
    def open_fragment_if_needed(self):
        if len(self.frag_q) == 0 and len(self.pending_fragments) > 0:
            ...
            # Close the current fragment, if present
            if self.current_fragment is not None:
                # TODO: instead, create a new copy of the stack, and append it to the new_children. So this block is not even needed.
                #  Rather, I need to modify the last block to create a next child and copy the open tag stack, if any.
                for t_name, _ in reversed(self.open_tag_stack):
                    self.new_content.append(f"</{t_name}>")
                self.new_content.append(f"</span>")
